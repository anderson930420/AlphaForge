...
- Remove any report-local assumptions that recompute storage layout from artifact filenames or folder naming when `SearchReportLinkContext` or explicit path inputs are available.
...
- Temporary state:
  - `report.py` may continue using `SearchReportLinkContext` plus `ArtifactReceipt` while report-link rendering is tightened.
